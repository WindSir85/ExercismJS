...
export function totalBirdCount(birdsPerDay) {
  ...
  for(let contador = 0; contador < birdsPerDay.length; contador++) {
    totalBirds = totalBirds + birdsPerDay[contador];
  }
  return totalBirds;
}
...
export function birdsInWeek(birdsPerDay, week) {
  ...
  for (let contador = weekStart; contador < weekEnd; contador++) {
    totalBirdsInWeek = totalBirdsInWeek + birdsPerDay[contador];
  }
...
export function fixBirdCountLog(birdsPerDay) {
  for (let contador = 0; contador < birdsPerDay.length; contador += 2) {
    let birdsValue = birdsPerDay[contador] + 1;
    birdsPerDay[contador] = birdsValue;
  }
  return birdsPerDay;
}
